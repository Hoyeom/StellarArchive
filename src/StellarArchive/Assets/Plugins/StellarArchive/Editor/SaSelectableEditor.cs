...
{
        [CustomEditor(typeof(SaSelectable), true)]
         public class SaSelectableEditor : Editor
         {
             ...
             GUIContent m_VisualizeNavigation = EditorGUIUtility.TrTextContent("Visualize", "Show navigation flows between Saselectable UI elements.");
             ...
             private static List<SaSelectableEditor> s_Editors = new List<SaSelectableEditor>();
             private static bool s_ShowNavigation = false;
             private static string s_ShowNavigationKey = "SaSelectableEditor.ShowNavigation";
     
             // Whenever adding new SerializedProperties to the SaSelectable and SaSelectableEditor
             // Also update this guy in OnEnable. This makes the inherited classes from SaSelectable not require a CustomEditor.
             ...
             protected virtual void OnEnable()
             {
                 ...
                 m_ShowColorTint.value       = (trans == SaSelectable.Transition.ColorTint);
                 m_ShowSpriteTrasition.value = (trans == SaSelectable.Transition.SpriteSwap);
                 m_ShowAnimTransition.value  = (trans == SaSelectable.Transition.Animation);
             ...
             static SaSelectable.Transition GetTransition(SerializedProperty transition)
             {
                 return (SaSelectable.Transition)transition.enumValueIndex;
             }
             ...
             public override void OnInspectorGUI()
             {
                 ...
                 if (graphic == null)
                     graphic = (target as SaSelectable).GetComponent<Graphic>();
     
                 var animator = (target as SaSelectable).GetComponent<Animator>();
                 ...
                 {
                     if (trans == SaSelectable.Transition.ColorTint || trans == SaSelectable.Transition.SpriteSwap)
                     {
                     ...
                     if (EditorGUILayout.BeginFadeGroup(m_ShowAnimTransition.faded))
                     {
                         ...
                         if (animator == null || animator.runtimeAnimatorController == null)
                         {
                             ...
                             {
                                 var controller = GenerateSaSelectableAnimatorContoller((target as SaSelectable).animationTriggers, target as SaSelectable);
                                 if (controller != null)
                                 {
                                     if (animator == null)
                                         animator = (target as SaSelectable).gameObject.AddComponent<Animator>();
                 ...
                 // We do this here to avoid requiring the user to also write a Editor for their SaSelectable-derived classes.
             ...
             // We need to make sure that m_PropertyPathToExcludeForChildClasses has all the SaSelectable properties and in the correct order.
             ...
             {
                 if (IsDerivedSaSelectableEditor())
                     return;
     
                 DrawPropertiesExcluding(serializedObject, m_PropertyPathToExcludeForChildClasses);
             }
     
             private bool IsDerivedSaSelectableEditor()
             {
                 return GetType() != typeof(SaSelectableEditor);
             }
     
             private static Animations.AnimatorController GenerateSaSelectableAnimatorContoller(AnimationTriggers animationTriggers, SaSelectable target)
             {
                 if (target == null)
                     return null;
             ...
             private static string GetSaveControllerPath(SaSelectable target)
             {
             ...
             private static string BuildAnimationPath(SaSelectable target)
             {
             ...
             private static void StaticOnSceneGUI(SceneView view)
             {
                 ...
                 for (int i = 0; i < selectables.Length; i++)
                 {
                     ...
                     if (SceneManagement.StageUtility.IsGameObjectRenderedByCamera(s.gameObject, Camera.current))
                         DrawNavigationForSaSelectable(s);
                 }
             }
     
             private static void DrawNavigationForSaSelectable(Selectable sel)
             {
                 if (sel == null)
                     return;
